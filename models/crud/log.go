...
//列表查询
func (s *LogCrud) GetAll(where []*db.QueryCondition, fields []string, orderBy string, page int, limit int) (*db.Paginator, error) {
	m := models.NewLog()
...
// 获取 单条记录
func (s *LogCrud) GetById(id int) (*models.Log, error) {
    m:=new(models.Log)
	m.Id = id
...
// 删除 单条记录
func (s *LogCrud) Delete(id int) (int64, error) {
	m:=new(models.Log)
	m.Id = id
