...
//列表查询
func (s *AreaCrud) GetAll(where []*db.QueryCondition, fields []string, orderBy string, page int, limit int) (*db.Paginator, error) {
	m := models.NewArea()
...
// 获取 单条记录
func (s *AreaCrud) GetById(id int) (*models.Area, error) {
    m:=new(models.Area)
	m.Id = id
...
// 删除 单条记录
func (s *AreaCrud) Delete(id int) (int64, error) {
	m:=new(models.Area)
	m.Id = id
