...
type Blog struct {
	...
	TimeSystem  time.Time `json:"time_system" xorm:"comment('创建时间,系统时间不可修改') TIMESTAMP"`
	TimeUpdate  time.Time `json:"time_update" xorm:"default 'CURRENT_TIMESTAMP' comment('更新时间') TIMESTAMP"`
	TimeAdd     time.Time `json:"time_add" xorm:"default 'CURRENT_TIMESTAMP' comment('添加时间,可修改') TIMESTAMP"`
