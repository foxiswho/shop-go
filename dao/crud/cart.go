...
//列表查询
func (s *CartCrud) GetAll(where []*db.QueryCondition, fields []string, orderBy string, page int, limit int) (*db.Paginator, error) {
	m := models.NewCart()
...
// 获取 单条记录
func (s *CartCrud) GetById(id int) (*models.Cart, error) {
    m:=new(models.Cart)
	m.Id = id
...
// 删除 单条记录
func (s *CartCrud) Delete(id int) (int64, error) {
	m:=new(models.Cart)
	m.Id = id
